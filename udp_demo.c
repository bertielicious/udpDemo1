...
void UDP_DEMO_Initialize(void)
{
    ...
    udpPacket.destinationAddress = MAKE_IPV4_ADDRESS(192,168,1,45); // IPv4 PC address
    udpPacket.sourcePortNumber = 65533;// Curiosity board port
    udpPacket.destinationPortNumber = 65531;// PC port
...
void UDP_DEMO_Send (void)
{
    ...
    potCurrResult =  (ADC_GetConversion(POT)/10);   // PG changed 'Pot' to 'POT' 2/8/22
